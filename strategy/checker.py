...
class CheckerJudgement:
    def __init__(self, status, points=None):
        self.status = status
        self.points = points
...
class Checker(Executable):
    ...
    def check(self, input, output, answer=None):
        ...
        return CheckerJudgement(judgement_properties["status"], float(judgement_properties["points"]) if "points" in judgement_properties else None)
